...
insert into public.tags (level, name, color)
  values ('major', '行事', '#ff0000') returning id \gset tagMajor15_
insert into public.tags (level, name, color, parent_id)
  values ('mid', '誕生日', '#00ff00', :'tagMajor15_id') returning id \gset tagMid15_
insert into public.tags (level, name, color, parent_id)
  values ('minor', '家族の誕生日', '#0000ff', :'tagMid15_id') returning id \gset tagMinor15_
...
select is(
  (select count(*) from public.list_past_events_by_tag(array[:'cal14_id'::uuid])),
  2::bigint,
...
select is(
  (select count(*) from public.list_past_events_by_tag(array[:'cal14_id'::uuid], :'tagMid15_id'::uuid)),
  1::bigint,
  '中分類タグで絞り込むと配下の小分類タグ付き過去予定が含まれること'
);
select is(
  (select title from public.list_past_events_by_tag(array[:'cal14_id'::uuid], :'tagMid15_id'::uuid)),
  '過去の誕生日会',
...
select is(
  (select count(*) from public.list_past_events_by_tag(array[:'cal14_id'::uuid], :'tagMajor15_id'::uuid)),
  1::bigint,
...
-- 無関係なタグでは該当0件
insert into public.tags (level, name, color)
  values ('major', '無関係タグ', '#123456') returning id \gset tagUnrelated15_
select is(
  (select count(*) from public.list_past_events_by_tag(array[:'cal14_id'::uuid], :'tagUnrelated15_id'::uuid)),
  0::bigint,
...
select throws_ok(
  format($$ select * from public.list_past_events_by_tag(array[%L::uuid]) $$, :'cal14_id'::uuid),
  '42501',
  null,
