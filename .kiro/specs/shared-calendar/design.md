...
**目的**: 本機能は、家族・カップル・友人グループなど複数人が予定を共有できるカレンダー基盤に、ToDo管理・階層タグ付け・タグ別の一覧振り返り・実施済み予定の思い出化・献立記録を統合したモバイルアプリケーションを提供する。

**利用者**: 日常の予定を複数人で共有・管理したいユーザーが、予定登録から準備(ToDo)、実施、振り返り(思い出・履歴)、日々の食事記録までを1つのアプリで完結させる。

**影響**: 新規グリーンフィールドプロダクトであり、既存システムへの影響はない。

### ゴール
- カレンダー共有・予定管理・コミュニケーション(コメント/スタンプ)というTimeTree相当の基盤機能を提供する
- 実施済み予定を写真・コメント付きの「思い出」として、繰り返し予定の各回も含めて個別に振り返り可能にする
- 予定に紐づくToDo・階層タグを管理し、タグ単位で過去の予定を横断的に振り返れるようにする
...
### 本specが担う範囲
- ユーザー認証、カレンダーの作成・共有・メンバー管理
- 予定(events)のCRUD、繰り返し予定の個別データ生成とまとめ操作、月/週/日表示
- 予定へのコメント・スタンプ
- 予定変更・コメント投稿・リマインドの通知配信と、リマインド対象者の選択
- 実施済み予定(繰り返しの各回を含む)の思い出化(写真・コメント)とタイムライン閲覧
- 予定に紐づくToDo管理とリマインド
- 予定用の階層タグ(大/中/小分類)とタグ絞り込みによる一覧振り返り
...
│   │   ├── calendars/            # カレンダーCRUD・招待・メンバー管理 (要件2、同一パターンを他featuresも踏襲)
│   │   ├── events/                # 予定CRUD・繰り返しシリーズ生成・まとめ操作 (要件3, 4)
│   │   ├── communication/        # コメント・スタンプ (要件5)
│   │   ├── tags/                  # 階層タグCRUD・絞り込みクエリ (要件10, 11)
│   │   ├── todos/                 # ToDo CRUD・リマインド設定 (要件9)
...
│       └── event-change-notifier/     # DB Webhook起動、変更・コメント通知 (要件6.1-6.3)
└── eas.json / app.json
```
...
**主な決定事項**: 通知起点を「DB Webhook(即時系)」と「pg_cronポーリング(時刻起点系)」の2系統に分離し、いずれも同一の`notification_log`で冪等性(重複配信防止)を担保する。リマインド系(6.4-6.6, 9.3)のみ`event_reminder_targets`による対象者絞り込みを行い、変更・コメント通知(6.1-6.3)は引き続き全メンバーへブロードキャストする。
...
| 2.1-2.6 | カレンダー作成・共有・メンバー管理 | CalendarService | Service, API(PostgREST) | - |
...
| 6.1-6.6 | 通知・リマインド対象者選択 | EventService(対象者設定・リマインダー時刻設定), NotificationDispatcher, EventChangeNotifier | Service, Event, Batch | 両通知フロー、繰り返し予定の生成・まとめ編集フロー |
| 7.1-7.6 | 実施済み予定の思い出化 | MemoryService | Service | 繰り返し予定の生成・まとめ編集フロー |
| 8.1-8.5 | 思い出の振り返り・閲覧 | MemoryService | Service | - |
| 9.1-9.6 | ToDo管理機能 | TodoService | Service, API | リマインドフロー |
| 10.1-10.8 | タグづけ機能(削除確認モーダル・カスケード削除を含む) | TagService | Service, API | - |
| 11.1-11.5 | 一覧振り返り機能 | HistoryService | Service | - |
...
| CalendarService | Identity/Membership | カレンダーCRUD・招待・メンバー管理 | 2.1-2.6 | AuthService (P0) | Service, API |
| EventService | Scheduling | 予定CRUD(削除確認含む)・繰り返しシリーズ生成(終了日必須・最大1年)/まとめ操作・リマインダー時刻/対象者設定・表示用クエリ | 3.1-3.9, 4.1-4.4, 6.4, 6.5 | CalendarService (P0) | Service, API |
| CommunicationService | Communication | コメント・スタンプのCRUD | 5.1-5.4 | EventService (P0) | Service, API |
| TagService | Tagging | 階層タグCRUD・予定への紐付け・削除確認とカスケード削除 | 10.1-10.8 | CalendarService (P0) | Service, API |
| TodoService | Todo | ToDo CRUD・リマインド設定 | 9.1-9.6 | EventService (P0) | Service, API |
| MemoryService | Memory | 実施済み予定(回ごと)の抽出・写真/コメント付与・思い出一覧 | 7.1-7.6, 8.1-8.5 | EventService (P0), Storage (P1) | Service |
| HistoryService | History | タグ絞り込みによる過去予定の横断検索 | 11.1-11.5 | EventService (P0), TagService (P0) | Service |
| MealRecordService | Meal | 献立記録CRUD・献立タグ管理 | 12.1-12.7 | CalendarService (P0) | Service, API |
| EventChangeNotifier | Notification | 予定変更・コメント投稿の即時通知 | 6.1-6.3 | EventService (P0), Expo Push (P0), Supabase service role (P0) | Event |
...
| 項目 | 内容 |
|------|------|
| 目的 | カレンダーの作成、招待コード発行、メンバー参加・削除、所属カレンダーの切り替えを担う |
| 対応要件 | 2.1, 2.2, 2.3, 2.4, 2.5, 2.6 |
...
**依存関係**
- Inbound: EventService, TagService, TodoService, MealRecordService — いずれも`calendar_id`の所属確認に依存(P0)
- Outbound: なし
- External: Supabase Auth — ユーザー同一性の確認(P0)
...
- 事後条件: `joinByInvite`成功時、呼び出しユーザーは`calendar_members`に`role: viewer`以上で追加される
- 不変条件: 1カレンダーに`owner`は最低1人存在する
...
| 対応要件 | 3.1, 3.2, 3.3, 3.4, 3.5, 3.6, 3.7, 3.8, 3.9, 4.1, 4.2, 4.3, 4.4, 6.4, 6.5 |
...
- 予定削除(`deleteEvent`)は、紐づく`todos`・`event_tags`・`event_reminder_targets`・`event_photos`・`event_comments`・`event_reactions`をカスケード削除する。クライアントは削除確定前に「思い出データ(写真・コメント)も削除されます」という確認モーダルを表示する(3.9、UI層の責務)
...
| 項目 | 内容 |
|------|------|
| 目的 | 予定用の階層タグ(大/中/小分類)のCRUD、予定への紐付け、削除時の確認とカスケード削除を管理する |
| 対応要件 | 10.1, 10.2, 10.3, 10.4, 10.5, 10.6, 10.7, 10.8 |
...
- 献立タグ(`meal_tags`)とは完全に独立したテーブル・APIとする
...
interface TagService {
  ...
  listTagTree(calendarId: string): Promise<Result<TagTreeNode[], TagError>>;
}
...
```
- 事前条件: `createTag`は`level`と`parent_id`のタグの`level`の整合性が取れていること(10.2)
- 事後条件: `attachTagsToEvent`は既存の紐付けを置き換えず追加する(10.5、複数タグ設定に対応)。`deleteTag`は対象タグ配下の子タグと、対象タグ・子タグに紐づく全ての`event_tags`をカスケード削除する(10.8)
- 不変条件: `major`タグの`parent_id`は常に`null`
...
**実装上の注意**
- 統合: `listTagTree`は再帰CTEで`major → mid → minor`をツリー形式に組み立ててクライアントへ返す
- 検証: 色は`#RRGGBB`形式のバリデーションをクライアント・DB双方で行う。タグ削除はクライアント側で確認モーダルを表示してからAPIを呼び出す(10.7)
- リスク: なし。カスケード削除は`tags.parent_id`と`event_tags.tag_id`の`ON DELETE CASCADE`制約により実装する(データモデル参照)
...
| 項目 | 内容 |
|------|------|
| 目的 | 予定に紐づくToDoのCRUD、達成状況管理、リマインド設定を提供する |
| 対応要件 | 9.1, 9.2, 9.3, 9.4, 9.5, 9.6 |

**責務と制約**
- `todos`テーブルの整合性を保つ唯一の書き込み境界。ToDoは必ず1件の`event_id`に紐づく(9.1)
- リマインドは`reminder_at`(絶対時刻)として保持し、配信自体はNotificationDispatcherが担当する。配信対象者は紐づく予定の`event_reminder_targets`設定を継承する(本コンポーネントは設定の保存のみ)

**依存関係**
- Inbound: NotificationDispatcher(リマインド対象の読み取り)(P0)
- Outbound: EventService — 紐付け先イベントの存在確認・対象者設定の参照(P0)
...
```
- 事前条件: `createTodo`は`eventId`が呼び出しユーザーの所属カレンダー内の予定であること
- 事後条件: `toggleDone`は`isDone`と`completedAt`を原子的に更新する
- 不変条件: `listTodosByCalendar`は完了/未完了を区別可能なフィールドを常に含む(9.5)
...
**実装上の注意**
- 統合: ToDo一覧画面(9.2)は`listTodosByCalendar`をカレンダー切り替えと連動させる
- 検証: `reminder_at`は`event`の`endAt`より過去に限定しない(準備ToDoは予定前のリマインドが主用途のため)
- リスク: なし(未解決事項に記載の項目以外)
...
| 項目 | 内容 |
|------|------|
| 目的 | カレンダー画面とは別に、過去の予定をタグ(階層問わず)で絞り込んで一覧表示する |
| 対応要件 | 11.1, 11.2, 11.3, 11.4, 11.5 |
...
interface HistoryService {
  listPastEventsByTag(calendarId: string, tagId?: string): Promise<Result<Event[], HistoryError>>;
}

type HistoryError = { type: "Forbidden" };
```
- 事前条件: なし
- 事後条件: `tagId`未指定時は全過去予定を日付順で返す。指定時は当該タグおよびその子孫タグが付与された予定のみを返す(11.2, 11.4)
...
| 項目 | 内容 |
|------|------|
| 目的 | `events`/`event_comments`のINSERT/UPDATE、および`event_series_creation_events`のINSERTをトリガーに、対象カレンダーの他メンバーへ即時プッシュ通知を送る |
| 対応要件 | 6.1, 6.2, 6.3 |

**責務と制約**
- DB Webhookからの呼び出しのみを受け付け、外部からの直接呼び出しは想定しない
- 通知対象は「操作者本人を除く」カレンダーメンバー全員(リマインド対象者設定の対象外、6.1-6.3は常に全メンバー)
...
##### イベントコントラクト
- 購読するイベント: `events`テーブルの`INSERT`(`series_id IS NULL`の場合のみ処理)/`UPDATE`、`event_comments`テーブルの`INSERT`、`event_series_creation_events`テーブルの`INSERT`(DB Webhookペイロード)
- 発行するイベント: Expo Push Notification Service への配信リクエスト(HTTPS)
- 順序・配信保証: at-least-once。`notification_log`の一意制約(event_id, target_user_id, type)で重複配信を検知しスキップする。`event_series_creation_events`起点の通知は`series_id`を`notification_log`の対象キーとして扱う
...
- 配信対象: `event_reminder_targets`に行が存在する予定はその対象者のみ、存在しない予定は当該カレンダーの全メンバー(6.5, 6.6)。ToDoのリマインドは紐づく予定の対象者設定を継承する
...
##### バッチ/ジョブコントラクト
- トリガー: pg_cronによる毎分起動
- 入力/検証: `todos.reminder_at <= now()` または `events.reminder_at <= now()`(いずれも`reminder_at IS NOT NULL`)を抽出し、`notification_log`未送信のもののみ処理。各対象について`event_reminder_targets`を参照し配信先を解決する(ToDoは紐づく予定の設定を継承)
...
### ドメインモデル
- **集約ルート**: `Calendar`(カレンダー) — `CalendarMember`, `Event`, `EventSeries`, `Tag`, `Todo`, `MealRecord`はすべて`calendar_id`で`Calendar`に従属する
- **Eventの子エンティティ**: `EventComment`, `EventReaction`, `EventPhoto`, `Todo`(1つのEventに複数のTodo)、`EventTag`(EventとTagの多対多)、`EventReminderTarget`(Eventとリマインド対象ユーザーの多対多)
- **EventSeriesとEventの関係**: 1つの`EventSeries`(繰り返し定義)から複数の`Event`(各回の実データ)が生成される。`Event.seriesId`は生成後不変
- **不変条件**: `Event.endAt >= Event.startAt`、`Todo.eventId`は必須、`Tag.level`と`Tag.parentId`の階層整合性、`MealRecord.slot`は4区分のいずれか
- **予定削除の波及**: `Event`削除時、`Todo`・`EventTag`・`EventReminderTarget`・`EventPhoto`・`EventComment`・`EventReaction`は全てカスケード削除される(3.9)。クライアントは削除確定前に思い出データが失われる旨を確認モーダルで明示する
...
**構造定義**:
- `calendars(id, name, created_by, created_at)`
...
- `tags(id, calendar_id, parent_id, level, name, color, created_at)`
- `event_tags(event_id, tag_id)` — 複合PK
- `todos(id, event_id, title, reminder_at, is_done, completed_at, created_by, created_at, updated_at)`
- `meal_records(id, calendar_id, meal_date, slot, title, rating, url, memo, created_by, created_at, updated_at)`
- `meal_tags(id, calendar_id, name, created_at)`
...
- `events.id`を参照する`todos.event_id`・`event_tags.event_id`・`event_reminder_targets.event_id`・`event_photos.event_id`・`event_comments.event_id`・`event_reactions.event_id`は全て`ON DELETE CASCADE`とする。予定削除は紐づく思い出データ(写真・コメント)も連鎖削除するため、クライアントは削除確定前に確認モーダルでその旨を明示する(3.9)
...
- 主要インデックス: `events(calendar_id, start_at)`(カレンダー表示・期間クエリ用)、`events(series_id)`(シリーズ一括操作用)、`events(reminder_at) WHERE reminder_at IS NOT NULL`(予定リマインド抽出用の部分インデックス)、`todos(event_id)`、`todos(reminder_at) WHERE is_done = false`(ToDoリマインド抽出用の部分インデックス)、`event_reminder_targets(event_id)`、`event_tags(tag_id)`、`tags(calendar_id, parent_id)`
- RLSポリシー: 全テーブルで`calendar_id`(または`event_id`経由の間接参照)を用い、`SECURITY DEFINER`関数`is_calendar_member(calendar_id, auth.uid())`を`USING`句・`WITH CHECK`句に適用する(詳細は[research.md](research.md))。Edge FunctionsはRLSをバイパスするservice role鍵で動作する(セキュリティに関する考慮事項を参照)
...
### エラー分類と対応
**ユーザーエラー(4xx)**: 未入力必須項目(タイトル未入力等) → フィールド単位のバリデーションエラー表示。権限なし(非メンバーのカレンダー操作) → 403相当の`Forbidden`でアクセス拒否画面へ誘導。
...
- 予定削除→カスケード削除フロー: `deleteEvent`実行時に紐づく`todos`・`event_tags`・`event_reminder_targets`・`event_photos`・`event_comments`が全て削除されること(3.9)
- 予定編集→通知フロー: `events`更新がDB Webhook経由で`EventChangeNotifier`を起動し`notification_log`に記録されること(6.2)
- リマインド対象者解決→配信フロー: `event_reminder_targets`設定時はその対象者のみ、未設定時は全メンバーに配信されること(6.5, 6.6)
...
- タグ絞り込み: `HistoryService.listPastEventsByTag`が中分類タグ指定時に配下の小分類タグ付き予定も含むこと(11.4)
